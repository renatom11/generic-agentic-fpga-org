...
- **Land the harvest locally** (ADR-0018): from each gate's collated harvest, dedup every tier-1/2 candidate against `docs/LESSONS.md` (land / merge-by-citation / drop-with-recurrence-note / contradiction → Supersedes fields, ADR-0010 as amended), transcribe what lands with self-contained incident descriptions — the file is the travel copy; a stranger holding only the file must be able to learn from every entry — and disposition every amendment obligation the round opened (DISCHARGED / NARRATIVE-ONLY / DEFERRED with named reason and discharging event), sweeping the board's DEFERRED backlog before the gate closes (PROTOCOL §7.1). Nothing transmits between repositories, and no lessons question ever reaches the sponsor.
