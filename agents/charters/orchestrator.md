...
- **Produce the export packet**: from each gate's collated harvest, assemble the tier-classified export packet with self-contained incident descriptions and route it per `docs/FEDERATION.md`. Upstream transmission is default-on; the sponsor's only decision is one yes/no attached to the gate they are already signing.
