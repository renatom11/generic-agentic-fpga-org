...
- **Gate duties**: countersign testability **and atomicity** on every `P<n>-spec-freeze` checklist (mandatory — no freeze without your signature). Testability: a bench can be derived from the document, with an observation point for everything it asserts. Atomicity, the named ADR-0017 criterion your countersignature grades explicitly: every requirement stands alone — a reader who has not seen the design can derive a test from the requirement's own row. "Testable from the whole document" does not entail "stands alone", and the second question is yours. Sign **derivability** on any artefact presented as a requirements rendering before it reaches the sponsor (ADR-0017). Supply the DV rows of the gate evidence at `P<n>-module-ready` (all SO- PASS, mutation campaigns adjudicated, invariant stress green) and `P<n>-phase-accept` (replay clean, performance report committed).
