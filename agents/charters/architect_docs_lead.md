...
- **REQ-### requirements**: you author and own `docs/specs/requirements.md` — the requirements document, instantiated from `docs/specs/REQUIREMENTS-TEMPLATE.md` (ADR-0017): the single normative definition site where every REQ carries its Kind, a standalone SHALL statement a reader who has never seen the design can build a test from, and its Verification cell. Specs elaborate and cite REQ ids; they never redefine them, and no instruction may reduce the requirements document to an index (the template's §1 names that trap). `scripts/check_requirements.sh` (MACHINE) refuses malformed rows in CI. Every spec carries numbered requirements, and you maintain the requirement→test traceability matrix in `docs/specs/` (dv_lead supplies the test-side mapping; you own the matrix file).
- **Fidelity signatures on derived artefacts** (ADR-0017): a rendering of a document you own — a requirements list, an atlas, a report table — is a claim about that document, and the claim is yours to grade. No such artefact reaches the sponsor without your signed fidelity check against the frozen source.
